...
4. Click the ⚙ in the side panel to fill in your details, import your resume PDFs, and pick a theme (light/dark/system plus five accent colors)

Requires Chrome 114+ (side panel API).

## How it stores your data

- **Text entries** live in `chrome.storage.sync` and follow your Chrome account across machines. One key per entry, so each gets its own 8KB quota; the settings page warns instead of silently failing if an answer is too long to sync.
- **Resume PDFs** live in `chrome.storage.local` (as data URLs), local to each machine, re-import once per computer.
- **Backup**: the settings page exports everything (entries + resumes) to a single JSON file, and imports it back.

Nothing is ever sent anywhere. There is no server, no analytics, no network request in the entire codebase. Storage is plain text, though, so don't put your SSN in it.
...
options/   settings page: edit, pin, reorder, resumes, themes, backup
src/
